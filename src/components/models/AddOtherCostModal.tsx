...
import { addOtherCost, editOtherCost } from "@/store/slices/otherCostSlice";
...
export default function AddOtherCostModal({
  open,
  onOpenChange,
  initialData,
  setInitialData,
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (initialData) {
      dispatch(
        editOtherCost({
          id,
          editedOtherCost: { id, amount: Number(amount), date: date!, title },
        }),
      );

      toast("বিবিধ খরচ পরিবর্তন হয়েছে", {
        description: `বিবরণ: ${newCost.title}`,
      });
    } else {
      dispatch(addOtherCost(newCost));

      toast("বিবিধ খরচ যোগ হয়েছে", {
        description: `বিবরণ: ${newCost.title}`,
      });
    }
    ...
    console.log("Other Cost added:", newCost);
  };
    ...
    >
      <DialogContent className="sm:max-w-[500px]">
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            <DialogTitle>
              বিবিধ খরচ {initialData ? "আপডেট" : "যোগ"} করুন
            </DialogTitle>
            <DialogDescription>
              বিবিধ খরচের বিস্তারিত {initialData ? "আপডেট" : "যোগ"} করুন। বিবিধ
              খরচ হল, যেমনঃ ওয়াইফাই বিল, খালা বিল, ময়লা বিল, পানি বিল, ভুষি
              ইত্যাদি।
            </DialogDescription>
          </DialogHeader>
          <div className="mt-5 grid gap-4">
            ...
            <div className="grid w-full gap-3">
              ...
              <Textarea
                ...
                placeholder="ওয়াইফাই বিল, খালা বিল, ময়লা বিল, পানি বিল, ভুষি"
                id="title"
                required
                value={title}
