...
export default function AddMarketerModal({
  open,
  onOpenChange,
  initialData,
  setInitialData,
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (initialData) {
      dispatch(
        editMarketer({
          id,
          editedMarketer: { id, amount: Number(amount), date: date!, title },
        }),
      );
      toast("বাজারকারি আপডেট হয়েছে", {
        description: `বিবরণ (নাম): ${newMarketer.title}`,
      });
    } else {
      dispatch(addMarketer(newMarketer));

      toast("বাজারকারি যোগ হয়েছে", {
        description: `বিবরণ (নাম): ${newMarketer.title}`,
      });
    }
    ...
    console.log("Marketer added:", newMarketer);
  };
    ...
    >
      <DialogContent className="sm:max-w-[500px]">
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            <DialogTitle>
              বাজার খরচ {initialData ? "আপডেট" : "যোগ"} করুন
            </DialogTitle>
            <DialogDescription>
              বাজার খরচের বিস্তারিত {initialData ? "আপডেট" : "যোগ"} করুন।
            </DialogDescription>
          </DialogHeader>
          <div className="mt-5 grid gap-4">
            ...
            <div className="grid w-full gap-3">
              <Label htmlFor="title">বিবরণ (নাম)</Label>
              <Textarea
                className="resize-none"
                placeholder="বাজারকারিদের নাম লিখুন"
                id="title"
                required
                value={title}
