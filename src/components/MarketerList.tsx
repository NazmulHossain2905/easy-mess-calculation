...
import AddMarketerModal from "./models/AddMarketerModal";
import { useAppDispatch, useAppSelector } from "@/store";
import { addMarketer, deleteMarketer } from "@/store/slices/marketerSlice";
...
const getColumns = (
  ...
  },
  {
    accessorKey: "title",
    header: ({}) => (
      <div className="font-bold text-blue-500">বিবরণ (বাজারকারিদের নাম)</div>
    ),
    cell: ({ row }) => (
  ...
  {
    accessorKey: "amount",
    header: () => <div className="font-bold text-fuchsia-600">খরচ</div>,
    cell: ({ row }) => (
...
export default function MarketerList() {
  ...
  const data = useAppSelector((state) => state.marketers.marketers);
  ...
  const handleDelete = (id: string) => {
    dispatch(deleteMarketer(id));
    setEditingMarketer(null);

    const findMarketer = data.find((marketer) => marketer.id === id);

    toast("বাজারকারি ডিলিট করা হয়েছে", {
      description: `বিবরণ (নাম): ${findMarketer?.title}`,

      action: {
        label: "Undo",
        onClick: () => dispatch(addMarketer(findMarketer!)),
      },
    });
  };
